...
### 新增 — 一键全链路 + 双级 OCR 降级
- **`court_full_pipeline.py`**：一键全链路脚本，输入短信 → 自动完成解析/下载/解析/归档/报告，1 回合替代 5-8 回合，积分消耗降低 70-80%
- **双级策略**：Tesseract（本地轻量 + 图片预处理）→ MinerU（云端高精度）
- **图片预处理**：Tesseract 前自动灰度化 + 1.5x 对比度增强 + 超大图缩放（PIL，零额外依赖）
- **质量门控**：`ocr_quality_score()` 综合评分，CJK≥10且占比≥20% +30、长度>50 +20、>200 +20、案号 +15、关键字 +15，≥50 通过
- **`run_ocr_tiered()`** 编排器：Tier 1→2 串行降级
- **`--ocr-tier` 参数**：tesseract / mineru
- **`ocr_meta` 输出**：JSON 含 `{tier, quality_score, acceptable, tiers_tried}`
- **图片校验**：格式白名单（PNG/JPG/BMP/TIFF）、文件大小检查（≤20MB）

### 改进
- MinerU 错误分类：区分网络不可达、API 认证失败、请求超时、通用错误
- MinerU 安装提示：不再静默下载，告知 ~30MB 下载量
- 质量门控收紧：CJK≥10 且占比 ≥20% 才给分（防垃圾字符误判）
- Tesseract 临时目录移到图片所在目录（解决沙箱隔离）

### 移除
- PaddleOCR 全部代码和引用（500MB+ 依赖，对法院文书 OCR 过度）

## [2.2.3] - 2026-07-13

### 修复
- macOS 不再生成桌面 `.ics` 备份文件（Apple Calendar 已原生写入，`.ics` 是冗余文件）
...
- 偏好文件按项目约定放入 `config/` 目录（`.example` 入 git，实际配置被 gitignore 忽略），与技能仓库规范保持一致
...
- 归档目录策略：未找到匹配案件时自动在桌面新建 `{原告}诉{被告} {案由}/`（内含 10 个标准子目录），不询问用户`（早期版本策略已废弃，以 SKILL.md 核心逻辑为准）`
...
- CHANGELOG 措辞修正：去除"AI 自然语言理解"和"替代正则解析"的说法，正则规则仍在 `sms-patterns.json` 中配合使用
